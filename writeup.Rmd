...
In this writeup, I will be analyzing data with machine learning.
...
1. Download data and remove any columns that we see will be problematic
2. Remove any columns from preprocessing
3. Fit 3 models (linear algebra, random forest, and boost)
4. Test their accuracy (out of sample error) against the cross validation set
5. Predict the 20 values from the test set
...
#Part 2 
First we want to see if linearcombos for any of the numeric data
```{r}
library(caret)
